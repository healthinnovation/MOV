...
h_prop<-
   ...
   arrange(orden,.by_group = T) %>% 
     ...
     # Calculo de hr
   mutate(pop= vacuna_ll/sum(vacuna_ll),
 ...
 data   
   ...
   arrange(orden,.by_group = T) %>% 
     ...
     # Calculo de hr
   mutate(pop= vacuna_hl/sum(vacuna_hl),
          #tasa = (vacuna/total)*10000,
          popr = cumsum(pop)/sum(pop),
          pop.popr = popr - pop,
          hr2 = round((pop.popr + popr)/2,3)*100
...
sii_rii<-function(data,level=NULL) {
   ...
   slice(2) %>%
    ...
    # rri f(1)/f(0)
   mutate(rii_l = map_dbl(.x = modelo,
                .f =~predict(.x,
                             newdata = bd %>% filter(hr2 == 100))/predict(.x,newdata = bd %>% filter(hr2 == 0))),
          
          ssi_l = estimate) %>% 
   
   select(vacunatipo,term,ssi_l,rii_l)
 ...
 data
   ...
   slice(2) %>%
    ...
    # rri f(1)/f(0)
   mutate(rii_h = map_dbl(.x = modelo,
                .f =~predict(.x,
                             newdata = bd %>% filter(hr2 == 100))/predict(.x,newdata = bd %>% filter(hr2 == 0))),
          
          ssi_h = estimate) %>% 
   
   select(vacunatipo,term,ssi_h,rii_h)
...
sii_rii_map<-function(data) {
   ...
   mutate(
      ...
      # lm para ssi 
     modelo = map(.x = data, .f = ~lm(formula = tasa~hr2, weights = total, data = .x)),
