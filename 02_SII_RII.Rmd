...
edu1<-
  ...
  as.data.frame() %>% 
  mutate(
    across(.cols = c(sii_l,sii_h,ssi), .fns = ~round(.x,4)*100),
    estimate = paste0(ssi," ","(",sii_l," ","-"," ",sii_h,")"),
    vacunatipo = case_when(vacunatipo == "PTV" ~ "Pentavalent",
                           vacunatipo == "ROTA" ~ "Rotavirus",
                           vacunatipo == "NEUMO" ~ "Pneumo",
                           vacunatipo == "INFLU" ~ "Influenza")
  )
...
# SII y RII por año (2010 - 2020)
 ...
 ssi.edu<-
  ...
  geom_rect(aes(xmin = 2015, xmax = Inf, ymin = -Inf, ymax = Inf), fill ="#edeeec")+
...
moran.ir<-
  data2 %>% 
   mutate(
    ...
    ANIO = ifelse(ANIO == "2015", paste0(ANIO,"*"),ANIO)
  ) %>% 
