...
options(survey.lonely.psu="remove")
 ...
 ggplot(data = MOV_GENERAL, aes(x = INDICERIQUEZA, fill = TIPORESIDENCIA)) +
...
#ggsave("test.png", width = 25, height = 14)
...
df4<-df3 %>% 
  ...
  mutate(
    ...
    grafico = map(.x = data,
                  .f = ~ggplot(data = .x %>% 
                                 mutate(siicat = ifelse(sii< -10,0,
                                                        ifelse(sii>-10&sii<0,1,
                                                               ifelse(sii>0&sii<10,2,3)))),
                               aes(fill=as.factor(siicat))) +
                    geom_sf(alpha = 0.7) +
                    scale_fill_manual(values = c("#404AA0","#817FB3","#F1EDFF","#00C894"),
                                      labels = c("< -10","-10 to 0","0 to 10","> 10"))+
...
cowplot::plot_grid(plotlist = df4$grafico, labels = c("PTV","NEUMO","ROTA","INFLU"))
ggsave("SII_RIQ_REGION.png",width = 19,height = 10)
...
# EDUCACION DE LA MADRE POR CRED
```{r}
df3<-
  df2 %>% 
  mutate(
...
# EDUCACION MADRE POR TIPO VIVIENDA
...
# INDICE RIQUEZA POR TIPO VIVIENDA
