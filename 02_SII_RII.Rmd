...
df3 %>%
  ...
  unnest(cols = c(hr)) %>%
  as.data.frame() %>%
  filter(EDU_MADRE%in%c("primaria","secundaria")) %>%

  ggplot()+
  geom_line(aes(x = as.factor(ANIO), y = tasa, col = EDU_MADRE, group=EDU_MADRE)) +
  facet_wrap(~vacunatipo, scale = "free")
...
temp<-df3 %>% 
  ...
  mutate(grafico = map(.x = data,
                       .f = ~ggplot(data = .x)+
  geom_col(aes(x = EDU_MADRE, y = tasa, fill = as.factor(ANIO), group = as.factor(ANIO)))+
  geom_point(aes(x = EDU_MADRE, y = tasa, fill = as.factor(ANIO), group = as.factor(ANIO)))+
  geom_smooth(aes(x = EDU_MADRE, y = tasa, fill = as.factor(ANIO), group = as.factor(ANIO)), method = "lm",se = F)+
  facet_wrap(~vacunatipo, scales = "free")))


cowplot::plot_grid(plotlist = temp$grafico)

ggsave("temp2.png", width = 18, height = 8)
...
df3 %>%
  ...
  unnest(cols = c(hr)) %>%
  as.data.frame() %>%
  filter(INDICERIQUEZA%in%c("1ro","5to")) %>%

  ggplot()+
  geom_line(aes(x = as.factor(ANIO), y = tasa, col = INDICERIQUEZA, group=INDICERIQUEZA)) +
  facet_wrap(~vacunatipo, scale = "free")
...
temp<-df3 %>% 
  ...
  mutate(grafico = map(.x = data,
                       .f = ~ggplot(data = .x)+
  geom_col(aes(x = INDICERIQUEZA, y = tasa, fill = as.factor(ANIO), group = as.factor(ANIO)))+
  geom_point(aes(x = INDICERIQUEZA, y = tasa, fill = as.factor(ANIO), group = as.factor(ANIO)))+
  geom_smooth(aes(x = INDICERIQUEZA, y = tasa, fill = as.factor(ANIO), group = as.factor(ANIO)), method = "lm",se = F)+
  facet_wrap(~vacunatipo, scales = "free")))


cowplot::plot_grid(plotlist = temp$grafico)

ggsave("temp.png", width = 18, height = 8)
