...
neumo1<-  
  df2 %>%
    mutate(
      ...
      neumo1ci = map(.x = neumo1,
                   .f = ~confint(.x) %>% 
                     as.data.frame() %>%
                     rownames_to_column("tar"))) %>% 
    unnest(cols = c(neumo1ci)) %>% 
    as.data.frame() %>% 
    mutate(ci_l = `2.5%`,
           ci_h = `97.5%`,
           vacuna = neumo1$S45NM1_1,
           vacunatipo = "neumo1") %>% 
    select(ANIO,vacuna,ci_l,ci_h,vacunatipo)
  ...
  mutate(
    ...
    rota1ci = map(.x = rota1,
                 .f = ~confint(.x) %>% 
                   as.data.frame() %>%
                   rownames_to_column("tar"))) %>% 
  unnest(cols = c(rota1ci)) %>% 
  as.data.frame() %>% 
  mutate(ci_l = `2.5%`,
         ci_h = `97.5%`,
         vacuna = rota1$S45RT1_1,
         vacunatipo = "rota1") %>% 
  select(ANIO,vacuna,ci_l,ci_h,vacunatipo)
...
rbind(ptv1,neumo1,influ1,rota1) %>% 
  #ungroup() %>% 
  ggplot(aes(x = as.factor(ANIO), y = vacuna*100, ymin = ci_l*100, ymax = ci_h*100, fill= vacunatipo, col = vacunatipo, group = vacunatipo))+
  geom_smooth(alpha = 0.3) +
  scale_fill_manual(values = c("#346751","#C84B31","#FFCC29","blue4"), 
                     labels = c("RT1","IF1","NM1","PTV1")) +
...
ggsave("1raDosisTotal.png",width = 12, height = 6)           
...
covertura.region %>%
  ...
  geom_point() + 
  geom_line()+
  scale_color_manual(values = c("#346751","#C84B31","#FFCC29"), 
...
covertura.region %>%
  ...
  geom_point() + 
  geom_line()+
  scale_color_manual(values = c("#346751","#C84B31"), 
